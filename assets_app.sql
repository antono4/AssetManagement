...
-- Users (password hash untuk: admin123, staff123 - lihat catatan di bawah)
-- Hash ini dihasilkan dari PHP: password_hash('admin123', PASSWORD_BCRYPT)
...
--  Hash di atas adalah PLACEHOLDER agar skema terdokumentasi. Karena hash BCrypt
--  bersifat acak, hash statis tidak akan cocok. Jalankan satu kali perintah ini
--  dari PHP (mis. php spark) untuk mengisi password yang benar:
--
...
--          \$db->query('UPDATE users SET password=? WHERE username=?',
--          [password_hash('staff123',PASSWORD_BCRYPT),'staff']);"
--
--  Sebagai alternatif tanpa PHP, gunakan aplikasi bagian "Setup" yang sudah
--  disediakan: akses  http://localhost:8080/setup  (route di Routes.php).
--  Route setup akan memperbarui kedua password di atas ke hash yang valid.
-- ============================================================================
